...
namespace SeleniumDemo
{
    public class seleniumHandler
    {
            ...
            for(int i =0; i< elementlist.Count;i++)
            {
                foreach(QueryObject QO in QOlist)
                {
                    if(elementlist[i].Text == QO.getQueryName())
                    {
                        ...
                        returncommand += QO.formUpdateCommandActual();
                    }
                }
            }
        ...
        static int Main(string[] args)
        {
            ...
            bool overRide = false;
            string DevProd = "Dev";
            if (doTests(user, password, overRide, DevProd, headOrHeadless, AppsToIgnore) == true)
            {
                return 0;
            }
            else return 1;
        ...
        public static bool doTests(string username, string pass, bool overRide, string DevProdstring, string Headless, string[] filters)
        {
            user = "tec\\"+username;
            password = pass;
            ...
            if (filters != null)
            {
                List<string> filterliststring = new List<string>(filters);
                try {
            ...
            MasterSqlTableObj themaster = new MasterSqlTableObj(DevProd, overRide, filterListints); //override default = false;
            string date = themaster.date;

            //  Dictionary<string, List<QueryObject>> theDictionary = themaster.url2QueriesDictionary;
            List<KPIpage> Kpipages = themaster.KPIpages;
            ...
            //foreach (KeyValuePair<string, List<QueryObject>> pair in theDictionary)
            foreach(KPIpage KP in Kpipages)
            {
                ...
                else
                {
                  ...
                  foreach(QueryObject QO in KP.getKPIs())
                    ...
                    // bigupdatestatement += mySelenium.ListKPIs(pair.Value);
            ...
            themaster.updateEverythingElse(bigupdatestatement);
            watch.Stop();
